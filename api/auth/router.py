...
@router.post("/register", status_code=201, description="Register new user", responses={409: {"description": "User already exists"}})
async def register(user: UserLogin):
    ...
    async with db.get_session() as session:
        ...
        session.add(UserModel(username=username, password=hash_password(password), email=email))
        await session.commit()
        return {"message": "User created"}


@router.post("/token", description="Get authentification token", responses={401: {"description": "Invalid username or password"}})
